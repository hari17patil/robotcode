...
Suite Setup     run keywords   setup tdc meetings      AND     Sign In     device_list=device_1:meeting_user,device_2:user,device_3:user
...
*** Keywords ***
setup tdc meetings
# Simple hack to skip all meeting creations - just uncomment the next 4 lines:
#    Log     ALL MEETING CREATIONS DISABLED
#
#never referenced
...
setup meeting user meetings
    ...
    ${passed} =    Run Keyword And Return Status  create TDC meeting on desktop    device=${device}     meeting_name=cnf_device_meeting    time_duration=20 hr     participants=device_2,device_3   use_current_time=True
...
setup regular user meeting
    [Arguments]     ${device}
    ${passed} =    Run Keyword And Return Status  delete all meetings from tdc        device=${device}
    ${meeting_id}    ${passcode} =  create TDC meeting on desktop       device=${device}     meeting_name=lock_meeting      time_duration=20 hr     participants=device_1:meeting_user,device_1:pstn_user,device_3  use_current_time=True       fetch_meeting_info=True
